...
{
  #logfile <- paste0(theFiles$fileName[x], ".log")
  ...
  rFn=paste0(localDir, "open-fvs-R-64-v", fvsVersion, "ForestVegetationSimulator-Interface/fvsOL/inst/extdata/AcadianGY_Grow.R")
  ...
  verbose=F # use F to suppress interactive messages during run
  if (verbose){
...
#                       cratio=round(as.numeric(newtree$CR)*100,2)))
  ...
  repeat
  {
    ...
    ops   = list(verbose=F,INGROWTH=INGROWTH,MinDBH=MinDBH,
                 CutPoint=0.5,   # >0 uses threshold probability (>0-1).
                 mortType="continuous", #mortType="discrete",
                 SBW=SBW,THINMOD=THINMOD,verbose=F,
    ...
    for (year in stdInfo["year"]:stdInfo["cendyear"])
    {
               ...
               any(is.na(treeout$HT)) || any(is.na(treeout$EXPF)))
      {
        ...
        #acd, Write the error message to a text file
        errorMsg_acd <- paste("AcadianGYOneStand failed in plot =", plot_location$Plot_ID[plot_location$FVSVariant == toupper(variant[i])][counter], "year =", year,"\n")
        if (class(treeout)!="try-error") {treeout="critical result contains NA values"
                                         cat ("dmpFile name=",dmpFile,"\n")
                                         errorMsg_acd <- c(errorMsg_acd, "critical result contains NA values\n")}
                                        else {errorMsg_acd <- c(errorMsg_acd, treeout)}
        cat(errorMsg_acd, file = "AcadianGY.Failure.txt", append=TRUE, sep = "\n")
...
cat ("in uiAcadian uiAcadianVolume=",
  ...
  #if (verbose){
  #cat("FVS Simulation Log for run ", x, "\n")
  # Write additional logging or output information as needed
 # cat("Output from FVS run ", x, " goes here.\n")
 # }

  # Close the sink connection if opened
  #if (!interactive()) {
  #  sink()
  #}

  # Delete the log file after the FVS simulation is done
  #unlink(logfile)
}
